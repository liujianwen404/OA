		...
		function buildMinuteTpl(cur_time){
			...
			for(var j = 0; j < dates.minute.length;j++){
				if(dates.minute[j]==temp){
					minute_html += '<li class="Hunter-minute active" tabindex="-1" onblur="timeClick()" data-minute="' + dates.minute[j] + '">' + dates.minute[j] + '</li>';
				}else{
					minute_html += '<li class="Hunter-minute" tabindex="-1" onblur="timeClick()" data-minute="' + dates.minute[j] + '">' + dates.minute[j] + '</li>';
				}
			}
