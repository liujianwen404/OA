...
import com.ruoyi.process.leave.domain.BizLeave;
...
//                        .singleResult();
    ...
    public ProcessInstance submitApply(BizLeaveVo entity, String applyUserId) {
        ...
        bizLeaveMapper.updateByPrimaryKey(entity);
        // 实体类 ID，作为流程的业务 key
        ...
        int days = dateService.selectDateCounts(entity.getStartTime(),entity.getEndTime());
        ...
        // 下一节点处理人待办事项
        bizTodoItemService.insertTodoItem(processInstanceId, entity.getTitle(), "leave",processInstance.getStartUserId());
    ...
    public void complete(BizLeaveVo leave, Boolean saveEntity, String taskId, Map<String, Object> variables) {
        ...
        ProcessInstance processInstance = runtimeService.createProcessInstanceQuery().processInstanceId(leave.getInstanceId()).singleResult();
        ...
        // 下一节点处理人待办事项
        bizTodoItemService.insertTodoItem(leave.getInstanceId(), leave.getTitle(), "leave",true,todoId,processInstance.getStartUserId());
    }
    ...
    public AjaxResult repeal(String instanceId, HttpServletRequest request, String message) {
        ...
        //维护业务数据
        Example example = new Example(BizLeave.class);
        example.createCriteria().andEqualTo("instanceId",instanceId);
        BizLeave leave = bizLeaveMapper.selectSingleOneByExample(example);
        leave.setAuditStatus("4");
        bizLeaveMapper.updateByPrimaryKeySelective(leave);
